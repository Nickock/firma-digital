...
export async function sendWelcomeEmail(email: string, verificationtoken = '123-123-321') {
  ...
  const options = {
    ...
    body: JSON.stringify({
      ...
      textContent:
        '¡Te damos la bienvenida SecureID! Tu registro se ha completado exitosamente!. Ahora verifica tu email.',
...
export async function sendEmail(email: string, messages: MailFormat, subject: string) {
  const url = 'https://api.brevo.com/v3/smtp/email'
  const options = {
    method: 'POST',
    headers: {
