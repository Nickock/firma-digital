...
import { SignRequestData, SignRequestSchema } from '../../schemas/ExternalAppSchema'
...
externalAppRouter.post('/sign-request', externalAppMiddleware(), async (req, res) => {
  ...
  const response = await ExternalAppController.createSignRequest(apiId, body.data as SignRequestData)
  if (response.error) {
...
externalAppRouter.get('/sign-request', async (req, res) => {
  ...
  const response = await SignRequestController.getSignRequestData(signId as string)
